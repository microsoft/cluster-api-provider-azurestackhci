...
func (r *AzureStackHCIVirtualMachineReconciler) reconcileNormal(virtualMachineScope *scope.VirtualMachineScope) (reconcile.Result, error) {
	...
	case infrav1.VMStateSucceeded:
		...
		if conditions.IsTrue(virtualMachineScope.Machine, clusterv1.MachineNodeHealthyCondition) {
			err = ams.removeISODisk()
			if err != nil {
				virtualMachineScope.Info("Failed to remove ISO", "error", err)
			}
		}
